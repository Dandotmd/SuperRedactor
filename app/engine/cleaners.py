...
MAX_SAMPLES = 3
# Below this, a record and a heading row look the same.
MIN_COLUMNS_FOR_TITLE_ROWS = 3
...
def _detect_title_rows(headers: list[str], rows: list[list[str]]):
    ...
    Only rows with at most one populated cell qualify, and only on sheets
    at least three columns wide. Anything else is left alone — a heading
    row with unnamed trailing columns is narrower than its data rows, and
    on a two-column sheet a real record is indistinguishable from a
    heading. Promoting one puts that record in the headings, where
    redaction can never reach it.
    """
    if len(headers) < MIN_COLUMNS_FOR_TITLE_ROWS:
        return None
